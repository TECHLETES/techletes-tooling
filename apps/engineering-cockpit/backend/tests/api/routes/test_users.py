...
def test_create_user_new_email(
    ...
    def capture_email(*, email_to: str, subject: str, html_content: str) -> None:
        sent_emails.append(
            {
                "email_to": email_to,
                "subject": subject,
    ...
    with (
        patch("backend.api.routes.users.send_email", side_effect=capture_email),
        ...
        assert sent_emails
        email = sent_emails[0]
        assert email["email_to"] == username
        assert "New account" not in email["subject"]
        assert "Password recovery" in email["subject"]
        assert password not in email["html_content"]
        assert "/reset-password?token=" in email["html_content"]
...
def test_delete_user_without_privileges(
    ...
    assert r.status_code == 403
    assert r.json()["detail"] == "User does not have required permission: users:delete"
